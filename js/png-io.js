...
function handleFileSelect(evt) {
    ...
    if (file) {
        ...
        reader.onload = function (evt) {
            var decoded = evt.target.result.replace(
...
function updateSaveImageUrl() {
    ...
    var svgxml = new XMLSerializer().serializeToString(
    ...
    xmlDoc
        ...
        .setAttribute("width", d3Graph.graph().width * targetScale);
    xmlDoc
        .getElementById("svg-canvas")
        .setAttribute("height", d3Graph.graph().height * targetScale);
    xmlDoc
        .getElementById("svgGroup")
        .setAttribute("transform", "translate(0,0)scale(" + targetScale + ")");
